...
python3 demo/visualize_tSNE.py \
--config-file demo/demo.yml \
--output demo/tsne \
DATALOADER.NUM_WORKERS 0 \
MODEL.WEIGHTS logs/AG_ReID/ViT/model_best.pth
'''

##-----------------------不是加入了视角的可视化，去fastreid/modeling/meta_arch/baseline_multi_view_SeCap.py修改测试输出-------------
if __name__ == '__main__':
    ...
    for i, (feat, pid, camid, viewid) in enumerate(tqdm.tqdm(demo.run_on_loader(test_loader), total=100)):
        if i > 4: break
        feats.append(feat)
        pids.extend(pid)
        camids.extend(camid)
    ...
    # # t-SNE 只能在 CPU 上跑 (sklearn 实现)
    # # n_jobs=-1 可以利用多核 CPU 加速
    # tsne = TSNE(n_components=2, learning_rate=200, metric='cosine', perplexity=30, random_state=1, n_jobs=-1)
    ...
    colors = plt.cm.nipy_spectral(np.linspace(0, 1, len(pid_container)))
