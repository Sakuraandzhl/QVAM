...
class Baseline_multiview_cargo(nn.Module):
    @configurable
    def __init__(
            self,
            *,
    ...
    def forward(self, batched_inputs, epoch=None):
        ...
        global_feats, patch_tokens = self.backbone(images, camids)
        ...
        if self.training:
            ...
            if self.qvam:
                ...
                return qvam_out['view_invariant_feats_enhance']
    ...
    def losses(self, label_id, label_view, global_out, view_invariant_feats_output, view_related_feats_outputs, qvam_out):
        loss_dict = {}
        ...
        if qvam_out:
            if 'view_related_feats' in qvam_out:
                loss_dict['loss_tri_rel'] = triplet_loss(qvam_out['view_related_feats'], label_view, tri_conf['margin'], tri_conf['norm_feat'], tri_conf['hard_mining']) * tri_conf['scale'] * ce_conf['view_lambda']
            ...
            loss_dict.setdefault('loss_global_mae_batch', 0.0)
            loss_dict.setdefault('loss_global_mae_to_memory1', 0.0)
            loss_dict.setdefault('loss_global_mae_to_memory2', 0.0)
            ...
            for uid in unique_ids:
                ...
                rel_feats = qvam_out['view_related_feats'][mask_id]
                with torch.no_grad():
                    for v in [0, 1]:
                ...
                if ground_mask.sum() > 0:
                    ...
                    rel_ground = rel_feats[ground_mask].mean(0)
                if aerial_mask.sum() > 0:
                    curr_aerial = curr_feats[aerial_mask].mean(0)
                    rel_aerial = rel_feats[aerial_mask].mean(0)
                if ground_mask.sum() > 0 and aerial_mask.sum() > 0:
                    margin_distinct = ce_conf['margin_distinct']
                    loss_dict['loss_global_mae_batch'] += F.pairwise_distance(curr_ground, curr_aerial).mean()
                    # loss_dict['loss_global_mae_batch'] += (margin_distinct - F.pairwise_distance(rel_ground, rel_aerial)).clamp(min=0)
                    cnt1 += 1
                ...
                if (proto_aerial.abs().sum() > 1e-6) and ground_mask.sum() > 0:
                    loss_dict['loss_global_mae_to_memory1'] += F.pairwise_distance(curr_ground, proto_aerial).mean()
                    cnt2 += 1
                if (proto_ground.abs().sum() > 1e-6) and aerial_mask.sum() > 0:
                    loss_dict['loss_global_mae_to_memory2'] += F.pairwise_distance(curr_aerial, proto_ground).mean()
                    cnt3 += 1
            
            if cnt1 > 0:
                loss_dict['loss_global_mae_batch'] = loss_dict['loss_global_mae_batch'] / cnt1 * ce_conf['cvpa_lambda']
            if cnt2 > 0:
                loss_dict['loss_global_mae_to_memory1'] = loss_dict['loss_global_mae_to_memory1'] / cnt2 * ce_conf['cvpa_lambda']
            if cnt3 > 0:
                loss_dict['loss_global_mae_to_memory2'] = loss_dict['loss_global_mae_to_memory2'] / cnt3 * ce_conf['cvpa_lambda']
