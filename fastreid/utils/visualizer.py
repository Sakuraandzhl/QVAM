...
class Visualizer:
    ...
    def save_rank_result(self, query_indices, output, max_rank=5, vis_label=False, label_sort='ascending',
        ...
        output_txt = 'right.txt' if 'vitvd' in output else 'wrong.txt'
        if os.path.exists(os.path.join(output, output_txt)):
            os.remove(os.path.join(output, output_txt))
        if vis_label:
        ...
        for cnt, q_idx in enumerate(tqdm.tqdm(query_indices)):
            ...
            if 'vitvd' in output:
                if cmc[0] == 1:
                    with open(os.path.join(output, output_txt), 'a') as f:
                        f.write(query_name + '\n')
                else:
                    continue
            else:
            ...
            for i in range(max_rank):
                ...
                if cmc[i] == 1:
                    ...
                    ax.add_patch(plt.Rectangle(xy=(0, 0), width=gallery_img.shape[1] - 1,
                                               ...
                                               edgecolor=(1, 0, 0), fill=False, linewidth=10))
                ax.imshow(gallery_img)
                ax.set_title(f'Rank{i + 1}')
                # ax.set_title(f'{self.sim[q_idx, sort_idx[i]]:.3f}/{label}/{gallery_name}/cam{cam_id}')
                ax.axis("off")
            # if actmap:
            #     act_outputs = []
            #
            ...
            filepath = os.path.join(output, "{}.jpg".format(query_name))
