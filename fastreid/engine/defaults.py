...
def default_argument_parser():
    ...
    parser.add_argument(
        "--config-file",      # 参数名
        default="",           # 默认值为空，需要用户指定
        metavar="FILE",       # 在帮助信息中显示为 FILE
        help="path to config file"  # 参数说明
    )

    # 是否从最近的 checkpoint 恢复训练
    parser.add_argument(
        "--resume",
        action="store_true",  # 无需赋值，只要出现该参数，值就为 True
    ...
    parser.add_argument(
        "--machine-rank",
        type=int,
        default=0,            # 默认主机器编号为 0
        help="the rank of this machine (unique per machine)"
    )

    # 自动生成一个相对固定的端口号，避免多用户端口冲突
    port = 2 ** 15 + 2 ** 14 + hash(os.getuid() if sys.platform != "win32" else 1) % 2 ** 14

    # 分布式训练通信地址（单机多卡默认 localhost，多机训练需改为主节点 IP）
    parser.add_argument(
        "--dist-url",
...
def default_setup(cfg, args):
    ...
    if comm.is_main_process() and output_dir:
        ...
        with PathManager.open(path, "w", encoding="utf-8") as f:
            f.write(cfg.dump())
